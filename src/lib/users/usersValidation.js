...
export const validateUsername = username => {
	if (!REGEX.USERNAME.test(username)) return "Sólo minusculas y números";
	if (REGEX.START_WITH_NUMBER.test(username))
		return "No puedo empezar por número";
	if (username.length < 6 || username.length > 15)
		return "Logintud entre 6 y 15";
};
...
export const validateName = name => {
	...
	for (const word of nameSplitted) {
		if (word.startsWith("-") || word.endsWith("-"))
			return "Uno de guiones incorrecto";
	}

	if (name.length < 2 || name.length > 30) return "Logintud entre 2 y 30";
};
